#!/usr/bin/env bash
# MODIFIED 2026-04-27: emit single-line zenity/kdialog fallbacks (F-8); chown -R root:root and prefer fakeroot before dpkg-deb (F-4)
...
if [[ -f \$local_electron_path ]]; then
	...
	if command -v electron &> /dev/null; then
		...
		# NOTE: written into the launcher heredoc; keep each dialog call on a
		# single line — '\\<newline>' continuations get eaten by the unquoted
		# heredoc, leaving multi-tab whitespace in the emitted launcher.
...
# Force root:root ownership across the staging tree before packaging.
# Without this, files end up owned by the build-host uid (e.g. 1001:1001),
# which becomes a local privilege-escalation surface once installed under
# /usr/lib/. Prefer running as root (CI), fall back to fakeroot locally.
echo 'Normalising ownership to root:root for packaging...'
if [[ $(id -u) -eq 0 ]]; then
	chown -R 0:0 "$package_root" || exit 1
	build_cmd=(dpkg-deb --build "$package_root" "$deb_file")
elif command -v fakeroot &> /dev/null; then
	# shellcheck disable=SC2016
	# Single quotes are intentional: $1/$2 must be expanded by the inner
	# bash -c, not the outer shell. _, $package_root, $deb_file are passed
	# as positional args to the inner bash.
	build_cmd=(fakeroot -- bash -c \
		'chown -R 0:0 "$1" && dpkg-deb --build "$1" "$2"' \
		_ "$package_root" "$deb_file")
else
	echo 'Error: not running as root and fakeroot is not installed.' >&2
	echo 'Install fakeroot (apt install fakeroot) or run this script' >&2
	echo 'as root so package files can be owned by root:root.' >&2
	exit 1
fi
